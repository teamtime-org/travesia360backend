...
public class GetTimeEntriesQueryHandler : IQueryHandler<GetTimeEntriesQuery, Result<IEnumerable<TimeEntryDto>>>
{
    ...
    public async Task<Result<IEnumerable<TimeEntryDto>>> HandleAsync(GetTimeEntriesQuery query, CancellationToken cancellationToken = default)
    {
        try
        {
            var pagedResult = await _unitOfWork.TimeEntries.GetPagedAsync(
                query.Page,
                query.PageSize,
                query.StartDate,
                query.EndDate,
